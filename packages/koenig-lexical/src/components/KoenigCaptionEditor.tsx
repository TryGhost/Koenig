...
const Placeholder = ({text = 'Type here'}: PlaceholderProps) => {
    return (
...
interface CaptionPluginProps {
    parentEditor: LexicalEditor;
}

function CaptionPlugin({parentEditor}: CaptionPluginProps) {
            ...
            return mergeRegister(
                ...
                editor.registerCommand(
                    ...
                    (event) => {
                        ...
                        (event as KeyboardEvent & {_fromCaptionEditor?: boolean})._fromCaptionEditor = true;
...
interface KoenigCaptionEditorProps {
    paragraphs?: number;
...
const KoenigCaptionEditor = ({paragraphs = 1, captionEditor, captionEditorInitialState, placeholderText, className = 'koenig-lexical-caption'}: KoenigCaptionEditorProps) => {
    ...
    return (
        ...
        >
            <KoenigComposableEditor
                className={className}
                markdownTransformers={MINIMAL_TRANSFORMERS as Transformer[]}
                placeholder={<Placeholder text={placeholderText} />}
            >
                <CaptionPlugin parentEditor={parentEditor} />
                <RestrictContentPlugin allowBr={false} paragraphs={paragraphs} />
                <EmojiPickerPlugin />
            </KoenigComposableEditor>
        </KoenigNestedComposer>
    );
