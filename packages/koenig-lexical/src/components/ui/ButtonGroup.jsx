...
export function IconButton({dataTestId, onClick, label, name, selectedName, Icon}) {
    ...
    return (
        <li className="mb-0">
            <button
                aria-label={label}
                className={`group relative flex h-7 w-8 cursor-pointer items-center justify-center rounded-lg hover:bg-grey-150 ${isActive ? 'bg-grey-150 text-green-600 dark:bg-grey-900 dark:text-white' : 'text-black dark:text-white' } ${Icon || 'text-[1.3rem] font-bold'}`}
