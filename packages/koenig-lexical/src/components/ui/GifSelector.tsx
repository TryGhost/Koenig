...
import {Gif, type GifData} from './file-selectors/Gif/Gif';
...
const GifSelector = ({onGifInsert, onClickOutside, updateSearch, columns, isLoading, isLazyLoading, error, changeColumnCount, loadNextPage, gifs, provider}: GifSelectorProps) => {
    ...
    function handleGifSelect(selectedGif: GifItem) {
                ...
                <div className="h-full overflow-auto px-6" onScroll={handleScroll}>
                    {
                        !error && !isSearchInProgress && (
                            <div className="flex gap-4">
                                {columns.map(column => (
                                    <section key={column[0]?.id} className="flex grow basis-0 flex-col justify-start gap-4">
                                        {column.map(gif => (
