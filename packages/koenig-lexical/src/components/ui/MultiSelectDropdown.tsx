...
function Item({item, selected, onChange}: {item: DropdownItem; selected: boolean; onChange: (item: DropdownItem) => void}) {
    let selectionClass = '';
...
export function MultiSelectDropdown({placeholder = '', items = [], availableItems = [], onChange, dataTestId, allowAdd = true}: MultiSelectDropdownProps) {
    ...
    const handleDeselect = (event: React.MouseEvent, selectedItem: DropdownItem) => {
        // Prevent losing focus when clicking an option
        event.preventDefault();
    ...
    const getItem = (item: unknown, selected: boolean) => {
        const typedItem = item as DropdownItem;
        return (
            <Item key={typedItem.name} item={typedItem} selected={selected} onChange={handleSelect}/>
        );
    };

    const selectedItems = items.map(item => ({name: item, label: item}));
    const nonSelectedItems = availableItems.map(item => ({name: item, label: item as React.ReactNode})).filter(
        ai => !selectedItems.some(ii => ii.name === ai.name)
    );

    const filteredItems: DropdownItem[] = nonSelectedItems.filter(item => item.name.toLocaleLowerCase().includes(filter.toLocaleLowerCase()));
    const prefixItem = '';
            ...
            {open && !!filteredItems.length && (
                <DropdownContainer>
                    ...
                    <KeyboardSelection
                        ...
                        getItem={getItem}
                        items={filteredItems}
                        onSelect={handleSelect as (item: unknown) => void}
                    />
                </DropdownContainer>
            )}
        </div>
