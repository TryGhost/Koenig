...
export function ToggleSetting({label, description, isChecked, onChange, dataTestId}) {
    return (
        <label className="flex w-full cursor-pointer items-start justify-between">
            <div>
                <div className="text-xs font-semibold tracking-normal text-grey-900 dark:text-grey-300">{label}</div>
                {description &&
                    <p className="mt-1 w-11/12 text-xs font-normal leading-[1.5] text-grey-700 dark:text-grey-600">{description}</p>
                }
            </div>
...
export function SliderSetting({label, onChange, max, min, value, defaultValue, description, dataTestId}) {
    return (
        <div className="my-2 flex w-full flex-col gap-1">
            ...
            {description &&
                <p className="mt-1 text-xs font-normal leading-[1.5] text-grey-700 dark:text-grey-600">{description}</p>
            }
        </div>
    );
}
...
export function InputSetting({label, hideLabel, description, onChange, value, placeholder, dataTestId, onBlur}) {
    return (
        <div className="flex w-full flex-col justify-between">
            <div className={hideLabel ? 'sr-only' : 'mb-1.5 text-xs font-semibold tracking-normal text-grey-900 dark:text-grey-300'}>{label}</div>
            <Input dataTestId={dataTestId} placeholder={placeholder} value={value} onBlur={onBlur} onChange={onChange} />
            {description &&
                <p className="text-xs font-normal leading-[1.5] text-grey-700 dark:text-grey-600">{description}</p>
            }
        </div>
    );
}
...
export function DropdownSetting({label, description, value, menu, onChange, dataTestId}) {
    return (
        <div className="flex w-full flex-col justify-between gap-1.5">
            <div className="text-xs font-semibold tracking-normal text-grey-900 dark:text-grey-300" data-testid={`${dataTestId}-label`}>{label}</div>
            <Dropdown
                dataTestId={dataTestId}
                menu={menu}
                value={value}
...
export function MediaUploadSetting({className, label, hideLabel, onFileChange, isDraggedOver, placeholderRef, src, alt, isLoading, errors = [], progress, onRemoveMedia, icon, desc = '', size, borderStyle, mimeTypes, isPinturaEnabled, openImageEditor, setFileInputRef}) {
    return (
        <div className={clsx(className)} data-testid="media-upload-setting">
            <div className={hideLabel ? 'sr-only' : 'mb-2 text-xs font-semibold tracking-normal text-grey-900 dark:text-grey-400'}>{label}</div>
