...
export function CodeEditor({code, language, updateCode, updateLanguage}: CodeEditorProps) {
    ...
    const onLanguageChange = React.useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const languageMap: Record<string, typeof javascript | typeof html | typeof css> = {
...
export function CodeBlockCard({captionEditor, captionEditorInitialState, code, darkMode, isEditing, isSelected, language, updateCode, updateLanguage}: CodeBlockCardProps) {
    if (isEditing) {
        return (
            <CodeEditor
                code={code}
