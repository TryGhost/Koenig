...
export async function initialize({page, uri = '/#/?content=false', force: _force = false}: {page: Page; uri?: string; force?: boolean}) {
    ...
    if (currentUrl === 'about:blank') {
        ...
        await page.evaluate(async ([navigateTo, force]) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any -- browser context: window has custom properties
...
async function exposeLexicalEditor(page: Page) {
    ...
    await page.evaluate(() => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- browser context: window has custom properties
        const w = window as any;
        w.lexicalEditor = el.__lexicalEditor;
        w.originalEditorState = (w.lexicalEditor as {getEditorState: () => unknown}).getEditorState();
    });
}
...
export async function getEditorState(page: Page) {
    return await page.evaluate(() => {
        ...
        return (window as any).lexicalEditor.getEditorState().toJSON();
    });
}
